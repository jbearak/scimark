...
This document specifies the requirements for adding a word count status bar item to the Scientific Markdown VS Code extension. The feature will display word count information in the VS Code status bar, showing either the word count of the selected text or the entire document when no selection is active. This feature will allow users to uninstall Microsoft's word count sample extension by providing equivalent functionality integrated directly into Scientific Markdown.
