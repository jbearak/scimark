...
- [ ] 1. Extend Frontmatter interface and parser
  - [ ] 1.1 Add `codeBackgroundColor`, `codeFontColor`, and `codeBlockInset` fields to the `Frontmatter` interface in `src/frontmatter.ts`
  ...
  - [ ] 1.2 Add parsing cases in `parseFrontmatter()` for the three new fields and their aliases
  ...
  - [ ] 1.3 Add serialization in `serializeFrontmatter()` using canonical field names only
    - Emit `code-background-color`, `code-font-color`, `code-block-inset` when present
    - _Requirements: 4.6, 8.6, 11.6_

  - [ ]* 1.4 Write property test: frontmatter round-trip preservation (Property 4)
  ...
  - [ ]* 1.5 Write property test: invalid frontmatter values are ignored (Property 5)
  ...
  - [ ]* 1.6 Write property test: alias round-trip normalization (Property 11)
...
- [ ] 3. Modify `stylesXml()` for conditional CodeBlock and CodeChar style generation
  - [ ] 3.1 Add constants for default code block styling values in `src/md-to-docx.ts`
  ...
  - [ ] 3.2 Extend `stylesXml()` signature to accept code block config parameter
    - Add `codeBlockConfig?: { background?: string; insetMode: boolean; codeFontColor?: string; codeBlockInset?: number }` parameter
    - _Requirements: 1.1, 1.2, 1.3, 1.4, 5.1_

  - [ ] 3.3 Implement shading mode CodeBlock style in `stylesXml()`
  ...
  - [ ] 3.4 Preserve inset mode CodeBlock style in `stylesXml()` when `insetMode` is true
    - Keep existing `w:ind` left/right, no `w:shd` or `w:pBdr`
    - _Requirements: 2.1, 2.2_

  - [ ] 3.5 Implement conditional CodeChar style in `stylesXml()`
  ...
  - [ ]* 3.6 Write property test: shading mode style structure (Property 1)
  ...
  - [ ]* 3.7 Write property test: spacing invariant across modes (Property 2)
  ...
  - [ ]* 3.8 Write property test: code font color in style run properties (Property 3)
  ...
  - [ ]* 3.9 Write property test: inline code shading in shading mode (Property 8)
  ...
  - [ ]* 3.10 Write property test: inline code has no shading in inset mode (Property 9)
    - **Property 9: Inline code has no shading in inset mode**
    - For `"none"` or `"transparent"` background, verify CodeChar has no `w:shd`
    - **Validates: Requirements 10.3**

  - [ ]* 3.11 Write property test: code-block-inset does not affect inline code (Property 10)
...
- [ ] 5. Modify paragraph emission for conditional code block spacing
  - [ ] 5.1 Add `codeShadingMode: boolean` to `DocxGenState` interface in `src/md-to-docx.ts`
    - _Requirements: 3.1, 3.2, 3.3_

  - [ ] 5.2 Update `convertMdToDocx()` to resolve code block config from frontmatter
  ...
  - [ ] 5.3 Update `generateParagraph()` code_block handling for conditional spacing
  ...
  - [ ]* 5.4 Write property test: uniform paragraph treatment in shading mode (Property 6)
  ...
  - [ ]* 5.5 Write property test: inset mode first/last paragraph spacing (Property 7)
...
- [ ] 7. Update DOCX-to-Markdown converter for round-trip preservation
  - [ ] 7.1 Update `convertDocx()` in `src/converter.ts` to preserve `codeBackgroundColor`, `codeFontColor`, and `codeBlockInset` from original frontmatter during round-trip
  ...
  - [ ]* 7.2 Write unit tests for round-trip preservation
...
- [ ] 8. Document frontmatter fields in specification
  - [ ] 8.1 Add `code-background-color`, `code-font-color`, and `code-block-inset` entries to the frontmatter field table in `docs/specification.md`
