...
graph TD
    ...
    F[syntaxes/scimark.json] --> G[Editor syntax highlighting]
    H[src/preview/scimark-plugin.ts] --> I[Markdown preview]
...
1. **Command**: `scimark.formatHighlight` — calls `wrapSelection(text, '==', '==')`, registered in `extension.ts`.
...
3. **TextMate grammar**: New `format_highlight` pattern in `syntaxes/scimark.json` matching `==…==` but NOT `{==…==}`. Uses a negative lookbehind for `{` and negative lookahead for `}`. The capture group uses `[^}=]+` to exclude `=` characters, ensuring multiple inline highlights on one line tokenize as separate spans.
4. **Preview**: New inline rule in `scimark-plugin.ts` that detects `==…==` (when not preceded by `{`) and renders as `<mark class="scimark-format-highlight">`. New CSS class with a yellow/amber background distinct from the purple CriticMarkup highlight.
...
*For any* non-empty text string that does not contain `==`, rendering `==text==` through the markdown-it plugin shall produce HTML output containing a `<mark>` element with the `scimark-format-highlight` CSS class and the original text as content.
