...
- [ ] 1. Bib Reverse Map and workspace scan removal
  - [ ] 1.1 Add `bibReverseMap` data structure and maintenance functions to `src/lsp/server.ts`
  ...
  - [ ] 1.2 Simplify `findPairedMarkdownUris` to use same-basename + reverse map only
  ...
  - [ ] 1.3 Update `revalidateMarkdownDocsForBib` to use the reverse map
    - Replace the `documents.all()` loop + `resolveBibliographyPath` per doc with a `getMarkdownUrisForBib()` lookup
    - _Requirements: 6.3_
  - [ ]* 1.4 Write unit tests for bib reverse map lifecycle
...
- [ ] 2. Targeted citation key scanner
  - [ ] 2.1 Implement `findUsagesForKey(text, key)` in `src/lsp/citekey-language.ts`
  ...
  - [ ] 2.2 Update `findReferencesForKey` in `src/lsp/server.ts` to use `findUsagesForKey`
    - Replace `scanCitationUsages(text).filter(...)` with `findUsagesForKey(text, key)`
    - _Requirements: 2.1, 2.3_
  - [ ]* 2.3 Write property test for targeted key scanner equivalence
...
- [ ] 4. Single-pass decoration extraction
  - [ ] 4.1 Implement `extractAllDecorationRanges` in `src/highlight-colors.ts`
  ...
  - [ ] 4.2 Update `updateHighlightDecorations` in `src/extension.ts` to call `extractAllDecorationRanges`
    ...
    - _Requirements: 3.1_
  - [ ]* 4.3 Write property test for single-pass decoration extraction equivalence
    - **Property 3: Single-Pass Decoration Extraction Equivalence**
    - **Validates: Requirements 3.1, 3.2, 3.3**

- [ ] 5. Two-pointer overlap check in extractHighlightRanges
  - [ ] 5.1 Replace `.some()` overlap check with two-pointer sweep in `extractHighlightRanges`
  ...
  - [ ]* 5.2 Write property test for two-pointer overlap exclusion equivalence
    - **Property 4: Two-Pointer Overlap Exclusion Equivalence**
    - **Validates: Requirements 4.1, 4.2**

- [ ] 6. Local citekey resolution at cursor
  - [ ] 6.1 Rewrite `findCitekeyAtOffset` in `src/lsp/citekey-language.ts` to use bounded local scan
  ...
  - [ ]* 6.2 Write property test for local citekey resolution equivalence
...
- [ ] 8. Streaming CriticMarkup preprocessor
  - [ ] 8.1 Rewrite `preprocessCriticMarkup` in `src/critic-markup.ts` to use segment collection
  ...
  - [ ]* 8.2 Write property test for streaming preprocessor equivalence
    - **Property 6: Streaming Preprocessor Equivalence**
    - **Validates: Requirements 7.1, 7.2**

- [ ] 9. Preview parser micro-optimizations
  - [ ] 9.1 Optimize `manuscriptMarkdownBlock` in `src/preview/manuscript-markdown-plugin.ts`
    - Replace `src.slice(pos, pos+3)` + `patterns.includes()` with direct charCode checks
    - _Requirements: 8.1, 8.2_
  - [ ] 9.2 Add posMax bounds checking to `parseManuscriptMarkdown` closing marker searches
    - After `indexOf` finds a closing marker, verify `endPos <= state.posMax` before accepting
    - _Requirements: 8.3_

- [ ] 10. TextMate grammar improvements
  - [ ] 10.1 Fix injection selector to exclude code scopes in `syntaxes/manuscript-markdown.json`
  ...
  - [ ] 10.2 Reorder patterns in `syntaxes/manuscript-markdown.json`
