...
- [ ] 2. Implement formatting function
  - [ ] 2.1 Add `wrapColoredHighlight(text, color)` to `src/formatting.ts`
    - Returns `{ newText: '==' + text + '=={' + color + '}', cursorOffset: undefined }`
    - _Requirements: 1.4, 2.4_

- [ ] 3. Update preview rendering
  - [ ] 3.1 Update `src/preview/mdmarkup-plugin.ts` to parse `==text=={color}` syntax
  ...
  - [ ] 3.2 Update CriticMarkup highlight rendering to use Comment_Gray
...
- [ ] 6. Update toolbar and menu structure
  - [ ] 6.1 Add 14 color commands to `package.json` contributes.commands
    - One command per color: `mdmarkup.formatHighlight_{color}` with title matching the color name
    - _Requirements: 1.3, 1.4_

  - [ ] 6.2 Add `markdown.highlightColor` submenu to `package.json` contributes.submenus
    - Label: "Highlight Color"
    - _Requirements: 1.2_

  - [ ] 6.3 Reorganize `markdown.formatting` menu in `package.json`
  ...
  - [ ] 6.4 Register color commands in `src/extension.ts`
...
- [ ] 7. Update TextMate grammar
  - [ ] 7.1 Add colored highlight pattern to `syntaxes/mdmarkup.json`
...
- [ ] 8. Update navigation support
  - [ ] 8.1 Extend `combinedPattern` regex in `src/changes.ts`
  ...
  - [ ]* 8.2 Write property tests for navigation (Properties 7-8)
...
- [ ] 9. Add configurable default highlight color setting
  - [ ] 9.1 Add `mdmarkup.defaultHighlightColor` to `package.json` contributes.configuration
    - Type: string enum with all 14 color identifiers
    - Default: "yellow"
    - _Requirements: 8.1, 8.4_

  - [ ] 9.2 Wire the setting into preview plugin and editor decorations
  ...
  - [ ]* 9.3 Write property test for configurable default (Property 9)
