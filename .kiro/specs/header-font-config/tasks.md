...
- [x] 1. Add shared utility functions and extend Frontmatter interface
  - [x] 1.1 Implement `parseInlineArray` and `normalizeFontStyle` utilities in `src/frontmatter.ts`
  ...
  - [x] 1.2 Extend the `Frontmatter` interface with new fields in `src/frontmatter.ts`
  ...
  - [x]* 1.3 Write property test: Inline array parsing equivalence (Property 1)
  ...
  - [x]* 1.4 Write property test: Font style normalization is canonical and idempotent (Property 2)
  ...
  - [x]* 1.5 Write property test: Invalid font styles are rejected (Property 3)
...
- [x] 2. Implement frontmatter parsing for new fields
  - [x] 2.1 Add parsing cases for `header-font`, `header-font-size`, `header-font-style` in `parseFrontmatter` in `src/frontmatter.ts`
  ...
  - [x] 2.2 Add parsing cases for `title-font`, `title-font-size`, `title-font-style` in `parseFrontmatter` in `src/frontmatter.ts`
  ...
  - [x] 2.3 Extend `title` parsing to support Inline_Array syntax in `parseFrontmatter` in `src/frontmatter.ts`
  ...
  - [x]* 2.4 Write property test: Invalid font sizes are filtered (Property 4)
  ...
  - [x]* 2.5 Write property test: Repeated keys use last occurrence (Property 5)
...
- [x] 3. Implement frontmatter serialization for new fields
  - [x] 3.1 Extend `serializeFrontmatter` in `src/frontmatter.ts` to emit new fields
  ...
  - [x]* 3.2 Write property test: Serialization format correctness (Property 7)
  ...
  - [x]* 3.3 Write property test: Parse–serialize–parse round-trip (Property 8)
...
- [x] 5. Extend font resolution and docx generation
  - [x] 5.1 Add `resolveAtIndex` helper and extend `FontOverrides` interface in `src/md-to-docx.ts`
  ...
  - [x] 5.2 Extend `resolveFontOverrides` in `src/md-to-docx.ts` to populate new fields
  ...
  - [x]* 5.3 Write property test: Array inheritance resolution (Property 6)
  ...
  - [x]* 5.4 Write property test: Font fallback to body font (Property 9)
  ...
  - [x]* 5.5 Write property test: header-font-size takes precedence over proportional scaling (Property 10)
  ...
  - [x]* 5.6 Write property test: Backward compatibility of existing font fields (Property 13)
...
- [x] 6. Update `stylesXml` for heading and title font/style overrides
  - [x] 6.1 Extend `headingRpr` helper and Title style in `stylesXml` in `src/md-to-docx.ts`
  ...
  - [x]* 6.2 Write property test: stylesXml heading and title output correctness (Property 11)
...
- [x] 7. Update `generateDocumentXml` for per-title-element run properties
  - [x] 7.1 Add per-title-element `<w:rPr>` in `generateDocumentXml` in `src/md-to-docx.ts`
...
- [x] 8. Update `applyFontOverridesToTemplate` for heading and title overrides
  - [x] 8.1 Extend template override logic in `applyFontOverridesToTemplate` in `src/md-to-docx.ts`
  ...
  - [x]* 8.2 Write property test: Template application preserves unmodified styles (Property 12)
...
- [x] 10. Implement docx→md extraction of heading and title font properties
  - [x] 10.1 Extend `convertDocx` in `src/converter.ts` to extract heading/title font properties from `word/styles.xml`
  ...
  - [x]* 10.2 Write property test: Full pipeline round-trip md→docx→md (Property 14)
...
- [x] 11. Add LSP completions for new frontmatter keys
  - [x] 11.1 Extend LSP completion handler in `src/lsp/server.ts`
...
- [x] 12. Update user-facing documentation
  - [x] 12.1 Update `docs/specification.md` with new frontmatter fields
