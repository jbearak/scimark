...
- Consecutive section block invariant: when any close sentinel (`landscapeClose` or `portraitClose`) is immediately followed by any open sentinel (`landscapeOpen` or `portraitOpen`), the open's section break is skipped to avoid an empty intermediate page. This applies to all combinations: landscape→landscape, portrait→portrait, landscape→portrait, portrait→landscape.
