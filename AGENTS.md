...
- Zotero grouped citations: Mixed groups are split — Zotero entries become a field code, non-Zotero entries become plain formatted text, missing keys appear as `@citekey` inline with a post-bibliography note. Configurable via `mixedCitationStyle` (`separate` default, `unified`)
