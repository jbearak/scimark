...
- DOCX commented text rendering: Group adjacent text runs by identical `commentIds` even when run formatting differs, emit one `{==...==}` block + one annotation sequence. Highlighted text inside a comment produces nested `{====text====}` syntax; the MD→DOCX parser handles this via `findClosingHighlightMarker` (depth-aware `==` matching) and inner-delimiter stripping in token processing
...
- CriticMarkup auto-closing pairs: `language-configuration.json` adds multi-character auto-closing pairs (`{>>`→`<<}`, `{++`→`++}`, etc.) for Markdown. Custom `autoClosingPairs` replace (not merge with) VS Code's built-in pairs for the language, so single-character pairs like `{`→`}` must be explicitly included. Multi-character pairs take precedence over single-character `{`→`}`, preventing unwanted extra characters when typing CriticMarkup delimiters
...
- LSP citekey local scan bounds: `findCitekeyAtOffset()` must not stop at newlines when the cursor is inside a bracketed citation segment; prefer nearest unclosed `[` and matching `]` within a bounded window so multi-line grouped citations like `[@a;\n@b]` resolve correctly
- LSP bib reverse-map recovery: On `.bib` create/change events, recheck open markdown docs not yet present in `docToBibMap` and backfill matching entries before validation; this closes the stale-diagnostics/references window for docs opened before their bibliography file existed, without workspace scans
- Decoration extractor parity: `extractAllDecorationRanges()` must preserve `extractHighlightRanges()` behavior for `==...==` inside non-highlight CriticMarkup spans (e.g., `{++...++}`); only Critic highlight spans `{==...==}` suppress nested format highlights
- MD→DOCX CriticMarkup recursive formatting: Parse inner Critic payloads with markdown-it inline parsing (with Critic/comment/citation/math/footnote rules disabled) and carry normalized inner text runs on `MdRun` (`innerRuns` / `oldRuns` / `newRuns`). Generate `w:ins`/`w:del`/substitution/highlight output from those structured runs so inner bold/italic/highlight formatting survives without breaking highlight+comment anchoring semantics
