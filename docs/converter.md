...
- **Tables**: DOCX→Markdown import prefers pipe tables for simple content, grid tables for multi-line cells (when original format was grid), and HTML tables as a final fallback or when the original format was HTML. The original table format (`pipe`, `html`, or `grid`) is stored via `MANUSCRIPT_TABLE_FORMATS` custom property to preserve the format on round-trip. Markdown→DOCX export accepts pipe tables, grid tables, and HTML tables (with `colspan` and `rowspan` support)
